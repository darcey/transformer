...
class BeamManager:
    ...
    def prune_finished(self):
        ...
        if finished_sents.any():
            ...
            self.cache.trim_finished_sents(finished_sents)
    ...
    def compute_next_token_probs(self):
        ...
        if finished_mask.any():
            ...
            self.cache.register_finished_mask(finished_mask)
            ...
            self.cache.register_finished_mask(None)
            next_token_probs = self.auto_fn(self.symbols.reshape(-1, self.seq_len)[:,-1:], self.seq_len-1, self.cache)[:,-1,:] # [batch*beam, vocab]
