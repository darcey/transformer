    ...
    def forward(self, q, k=None, v=None, mask=None, cache=None):
        ...
        if cache is None:
            ...
            if k is not None:
                ...
                cache.cache_k(id(self), k_cache)
                cache.cache_v(id(self), v_cache)
            k = k_cache
            v = v_cache

        # reshape to heads, permute for matrix multiplication
        q = q.reshape(batch, -1, self.num_heads, self.qk_dim).permute((0,2,1,3)) # [batch, head, seq1, d_qk]
        k = k.reshape(batch, -1, self.num_heads, self.qk_dim).permute((0,2,3,1)) # [batch, head, d_qk, seq2]
        v = v.reshape(batch, -1, self.num_heads, self.v_dim).permute((0,2,1,3))  # [batch, head, seq2, d_v]
        if mask is not None:
            mask = mask.unsqueeze(1)                                             # [batch, 1, seq1 (or 1), seq2]

        # do multihead attention
        key_queries = torch.matmul(q,k)/math.sqrt(self.qk_dim)                   # [batch, head, seq1, seq2]
        if mask is not None:
            key_queries += mask
...
class InputLayer(torch.nn.Module):
    ...
    def forward(self, seq, timestep=0):
        ...
        seq_embed += self.positional(seq_embed, timestep=timestep)
        return seq_embed
...
class TransformerTwoSeq(torch.nn.Module):
    ...
    def get_autoregressive_one_step_fn(self, src, cache):
        ...
        for layer in self.decoder.layers:
            ...
            cache.cache_k(id(self_att), torch.empty(batch_size, 0, self_att.num_heads*self_att.qk_dim).type(src.type()))
            cache.cache_v(id(self_att), torch.empty(batch_size, 0, self_att.num_heads*self_att.qk_dim).type(src.type()))
...
class TransformerOneSeq(torch.nn.Module):
    ...
    def get_autoregressive_one_step_fn(self, cache, batch_size, device):
        ...
        for layer in self.xxcoder.layers:
            self_att = layer.self_attention
            cache.cache_k(id(self_att), torch.empty(batch_size, 0, self_att.num_heads*self_att.qk_dim, device=device))
            cache.cache_v(id(self_att), torch.empty(batch_size, 0, self_att.num_heads*self_att.qk_dim, device=device))
