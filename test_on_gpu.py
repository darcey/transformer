...
class TestGeneratorWorksOnGPU(unittest.TestCase):
    ...
    def testBeamSearchSameOnGPU(self):
        ...
        dist_cpu = torch.nn.functional.softmax(torch.rand(30,40,50), dim=-1)
        dist_gpu = dist_cpu.clone().cuda()

        def auto_fn_cpu(symbols, timestep, cache):
            return dist_cpu[:symbols.size(0),timestep:timestep+symbols.size(1),:]
        def auto_fn_gpu(symbols, timestep, cache):
            return dist_gpu[:symbols.size(0),timestep:timestep+symbols.size(1),:]
...
class TestCacheSameOnGPU(unittest.TestCase):
    ...
    def testCacheAndRetrieveSameOnGPU(self):
        if not torch.cuda.is_available():
            return

        src_mask_cpu = torch.rand(5,1,6)
        src_mask_gpu = src_mask_cpu.cuda()
        tgt_mask_cpu = torch.rand(5,1,9)
        tgt_mask_gpu = tgt_mask_cpu.cuda()
        k1_cpu = torch.rand(5,6,7)
        k1_gpu = k1_cpu.cuda()
        k2_cpu = torch.rand(5,6,7)
        k2_gpu = k2_cpu.cuda()
        v1_cpu = torch.rand(5,9,8)
        v1_gpu = v1_cpu.cuda()
        v2_cpu = torch.rand(5,9,8)
        v2_gpu = v2_cpu.cuda()

        finished_mask_cpu = torch.empty(5).uniform_() > 0.5
        finished_mask_gpu = finished_mask_cpu.cuda()

        cache_cpu = BeamCache(5,1)
        cache_gpu = BeamCache(5,1)
        cache_cpu.cache_src_mask(src_mask_cpu)
        cache_gpu.cache_src_mask(src_mask_gpu)
        cache_cpu.cache_tgt_mask(tgt_mask_cpu)
        cache_gpu.cache_tgt_mask(tgt_mask_gpu)
        cache_cpu.cache_k(1, k1_cpu)
        cache_gpu.cache_k(1, k1_gpu)
        cache_cpu.cache_k(2, k2_cpu)
        cache_gpu.cache_k(2, k2_gpu)
        cache_cpu.cache_v(1, v1_cpu)
        cache_gpu.cache_v(1, v1_gpu)
        cache_cpu.cache_v(2, v2_cpu)
        cache_gpu.cache_v(2, v2_gpu)

        src_mask_out_cpu = cache_cpu.get_src_mask()
        src_mask_out_gpu = cache_gpu.get_src_mask()
        tgt_mask_out_cpu = cache_cpu.get_tgt_mask()
        tgt_mask_out_gpu = cache_gpu.get_tgt_mask()
        k1_out_cpu = cache_cpu.get_k(1)
        k1_out_gpu = cache_gpu.get_k(1)
        k2_out_cpu = cache_cpu.get_k(2)
        k2_out_gpu = cache_gpu.get_k(2)
        v1_out_cpu = cache_cpu.get_v(1)
        v1_out_gpu = cache_gpu.get_v(1)
        v2_out_cpu = cache_cpu.get_v(2)
        v2_out_gpu = cache_gpu.get_v(2)
        self.assertTrue(torch.equal(src_mask_out_cpu, src_mask_out_gpu.cpu()))
        self.assertTrue(torch.equal(tgt_mask_out_cpu, tgt_mask_out_gpu.cpu()))
        self.assertTrue(torch.equal(k1_out_cpu, k1_out_gpu.cpu()))
        self.assertTrue(torch.equal(k2_out_cpu, k2_out_gpu.cpu()))
        self.assertTrue(torch.equal(v1_out_cpu, v1_out_gpu.cpu()))
        self.assertTrue(torch.equal(v2_out_cpu, v2_out_gpu.cpu()))
    ...
    def testExpandToBeamSizeSameOnGPU(self):
        ...
        tgt_mask_cpu = torch.rand(5,1,9)
        tgt_mask_gpu = tgt_mask_cpu.cuda()
        k1_cpu = torch.rand(5,6,8)
        k1_gpu = k1_cpu.cuda()
        k2_cpu = torch.rand(5,6,8)
        k2_gpu = k2_cpu.cuda()
        v1_cpu = torch.rand(5,9,8)
        v1_gpu = v1_cpu.cuda()
        v2_cpu = torch.rand(5,9,8)
...
class TestTransformerSameOnGPU(unittest.TestCase):
    ...
    def testTransformerTwoSeq(self):
        ...
        torch.testing.assert_close(out_cpu, out_gpu.to("cpu"), atol=0.0001, rtol=0)
        ...
        out_cpu = torch.empty(5,0,1000)
        out_gpu = torch.empty(5,0,1000).cuda()
        for i in range(0,20):
            out_cpu_one_symb = auto_fn_cpu(y_cpu[:,i:i+1], i, cache_cpu)
            out_gpu_one_symb = auto_fn_gpu(y_gpu[:,i:i+1], i, cache_gpu)
            out_cpu = torch.cat((out_cpu, out_cpu_one_symb), dim=1)
            out_gpu = torch.cat((out_gpu, out_gpu_one_symb), dim=1)
        torch.testing.assert_close(out_cpu, out_gpu.to("cpu"), atol=0.0001, rtol=0)
